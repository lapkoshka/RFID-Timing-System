...
namespace RFID_Timing
{
    class DeviceManager
    {
        ...
        private bool search()
        {
            ...
            if (searchErrorCode == DevControl.tagErrorCode.DM_ERR_OK)
            {
                return true;
            }
            else
        ...
        private void searchHandler(IntPtr dev, IntPtr data)
        {
            ...
            DevControl.tagErrorCode eCode = DevControl.DM_GetDeviceInfo(dev, ref this.ip, macAdd, devname);
        }
        ...
        private bool connect()
        {
            ...
            byte fComAdr = 255;
            int FrmPortIndex = 0;
            int fCmdRet = RWDev.OpenNetPort(nPort, ipAddress, ref fComAdr, ref FrmPortIndex);
            if (fCmdRet == 0)
            {
                RWDev.InitRFIDCallBack(new RFIDCallBack(RFIDTagCallback), true, FrmPortIndex);
                return true;
            }
            else
            {
        ...
        protected virtual void RFIDTagCallback(IntPtr p, Int32 nEvt)
        {
            ...
            EventHandler<TagCatchEventArgs> handler = tagCatch;
        ...
        public void start()
        {
            Thread readerWorker = new Thread(listenReader);
            readerWorker.IsBackground = true;
            readerWorker.Start();
        }
    ...
    public class TagCatchEventArgs : EventArgs
    {
